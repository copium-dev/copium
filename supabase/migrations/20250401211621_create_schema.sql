-- these are NOT public API so service, but note that service can bypass RLS
...
CREATE TABLE IF NOT EXISTS service.user_analytics (
    email TEXT PRIMARY KEY NOT NULL,
...
CREATE TABLE IF NOT EXISTS service.user_applications (
...
CREATE TABLE IF NOT EXISTS service.application_history (
    ...
    CONSTRAINT fk_application_history_app
        ...
        REFERENCES service.user_applications(application_id)
        ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_user_applications_email_applied_date_appid
    ON service.user_applications (email, applied_date DESC, application_id);
...
CREATE INDEX IF NOT EXISTS idx_application_history_email_appid_event_time
    ON service.application_history (email, application_id, event_time DESC);
...
CREATE INDEX IF NOT EXISTS idx_application_history_email_applied_date
    ON service.application_history (email, applied_date DESC)
    WHERE operation != 'revert';

CREATE INDEX IF NOT EXISTS idx_user_email
    ON service.users (email);

CREATE INDEX IF NOT EXISTS idx_user_analytics_email
    ON service.user_analytics (email);